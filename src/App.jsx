...
const cfg=getAppConfig();
const[params,setParams]=useState(()=>JSON.parse(localStorage.getItem("aco_fact_params_v45")||localStorage.getItem("aco_fact_params_v44")||localStorage.getItem("aco_fact_params_v43")||localStorage.getItem("aco_fact_params_v41")||"null")||{iva:cfg.iva||"21",retencio:cfg.retencio||"0",descompte:cfg.descompte||"0"});
useEffect(()=>localStorage.setItem("aco_fact_params_v45",JSON.stringify(params)),[params]);
let certs=(data.certificacions||[]).map(c=>({...c,base:calcEditedCertAmount(data,+c.numero)||(+c.import||0)}));
function upd(k,v){setParams(p=>({...p,[k]:v}))}
function calc(f){let b=+f.base||0,d=b*(+params.descompte||0)/100,sub=b-d,iva=sub*(+params.iva||0)/100,ret=sub*(+params.retencio||0)/100;return {b,d,sub,iva,ret,total:sub+iva-ret}}
function save(){localStorage.setItem("aco_fact_params_v45",JSON.stringify(params));alert("Paràmetres de facturació guardats")}
return <div className="stack"><Card title="Paràmetres factura / proforma" action={<button className="primary" onClick={save}><Save/> Guardar paràmetres</button>}>
<div className="invoice-controls"><label>IVA<select value={params.iva} onChange={e=>upd("iva",e.target.value)}><option value="0">0%</option><option value="10">10%</option><option value="21">21%</option></select></label><label>Retenció<select value={params.retencio} onChange={e=>upd("retencio",e.target.value)}><option value="0">0%</option><option value="7">7%</option><option value="15">15%</option><option value="19">19%</option></select></label><label>Deducció<select value={params.descompte} onChange={e=>upd("descompte",e.target.value)}><option value="0">0%</option><option value="5">5%</option><option value="10">10%</option><option value="15">15%</option></select></label></div><p className="mail-help">Email emissor configurat: <b>{cfg.email||"pendent de configurar"}</b>. L'enviament obre Gmail web.</p></Card>
<Card title="Factures proforma de certificacions">{certs.length===0?<Empty text="Encara no hi ha certificacions."/>:<div className="invoice-table">{certs.map(f=>{let x=calc(f);return <div className="invoice-row print-area" key={f.id}><div><b>Proforma {f.numero}</b><span>Certificació {f.numero} · {f.data}</span></div><span>Base<br/><b>{money(x.b)}</b></span><span>Deducció {params.descompte}%<br/><b>{money(x.d)}</b></span><span>IVA {params.iva}%<br/><b>{money(x.iva)}</b></span><span>Retenció {params.retencio}%<br/><b>-{money(x.ret)}</b></span><span>Total<br/><b>{money(x.total)}</b></span><div className="row-actions no-print"><button className="secondary" onClick={()=>openDoc({type:"proforma",title:`Proforma ${f.numero}`,subtitle:`Certificació ${f.numero} · ${f.data}`,proforma:f,iva:params.iva,ret:params.retencio,ded:params.descompte,total:x.total,linies:[`Base: ${money(x.b)}`,`Deducció (${params.descompte}%): ${money(x.d)}`,`IVA (${params.iva}%): ${money(x.iva)}`,`Retenció (${params.retencio}%): -${money(x.ret)}`,`Total: ${money(x.total)}`]})}>Obrir</button><button className="primary" onClick={()=>window.print()}>Imprimir</button><button className="secondary" onClick={()=>gmailSend(`Proforma certificació ${f.numero}`,`Adjunto/consulta proforma de la certificació ${f.numero}.\nBase: ${money(x.b)}\nDeducció: ${money(x.d)}\nIVA: ${money(x.iva)}\nRetenció: -${money(x.ret)}\nTotal: ${money(x.total)}`)}>Enviar</button></div></div>})}</div>}</Card></div>}
