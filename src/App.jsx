...
function num2(n){return new Intl.NumberFormat("ca-ES",{minimumFractionDigits:2,maximumFractionDigits:2}).format(+n||0)}
...
{screen==="Clients"&&<Clients clients={fClients} cs={cs} setCs={setCs} ct={ct} setCt={setCt} openClient={openClient} newClient={()=>setModal("client")} setClients={setClients}/>}
...
useEffect(()=>{localStorage.setItem("aco_clients_v41",JSON.stringify(list)); if(setClients) setClients(list)},[list]);
function add(){let n={id:"client-"+Date.now(),nom:"Nou client",rao:"Pendent",tipus:"Promotor",contacte:"Pendent",nif:"Pendent",email:"",telefon:"",adreca:"",logo:"",color:"blue"};setList(p=>[n,...p]);setEditing(n.id)}
function upd(id,k,v){setList(p=>p.map(c=>c.id===id?{...c,[k]:v}:c))}
function del(id){if(confirm("Segur que vols eliminar aquest client?"))setList(p=>p.filter(c=>c.id!==id))}
let filtered=list.filter(c=>(c.nom+" "+c.rao+" "+c.tipus+" "+c.nif+" "+c.email).toLowerCase().includes((cs||"").toLowerCase()) && (!ct||c.tipus===ct));
return <Card title="Clients" action={<button className="primary" onClick={add}><Plus/> Nou client</button>}>
<div className="filters"><div className="search-field"><Search size={16}/><input value={cs} onChange={e=>setCs(e.target.value)} placeholder="Buscar client..."/></div><select value={ct} onChange={e=>setCt(e.target.value)}><option value="">Tots</option><option>Promotor</option><option>Industrial</option><option>Constructora</option><option>Particular</option><option>Arquitecte</option><option>Arquitecte tècnic</option><option>Altres</option></select></div>
<div className="client-table-v41"><div className="client-head"><span>Nom</span><span>Tipus / rol</span><span>NIF</span><span>Email</span><span>Telèfon</span><span>Accions</span></div>
{filtered.map(c=>{let edit=editing===c.id;return <div className="client-line" key={c.id}>{edit?<>
<input value={c.nom||""} onChange={e=>upd(c.id,"nom",e.target.value)}/>
<select value={c.tipus||"Promotor"} onChange={e=>upd(c.id,"tipus",e.target.value)}><option>Promotor</option><option>Industrial</option><option>Constructora</option><option>Particular</option><option>Arquitecte</option><option>Arquitecte tècnic</option><option>Altres</option></select>
<input value={c.nif||""} onChange={e=>upd(c.id,"nif",e.target.value)}/>
<input value={c.email||""} onChange={e=>upd(c.id,"email",e.target.value)}/>
<input value={c.telefon||""} onChange={e=>upd(c.id,"telefon",e.target.value)}/>
<div className="row-actions"><button className="secondary" onClick={()=>setEditing(null)}>Guardar</button><button className="danger" onClick={()=>del(c.id)}>Eliminar</button></div>
<input className="client-address" value={c.adreca||""} onChange={e=>upd(c.id,"adreca",e.target.value)} placeholder="Adreça completa"/>
</>:<>
<strong>{c.nom}</strong><span>{c.tipus}</span><span>{c.nif||"—"}</span><span>{c.email||"—"}</span><span>{c.telefon||"—"}</span><div className="row-actions"><button className="secondary" onClick={()=>setEditing(c.id)}>Editar</button><button className="secondary" onClick={()=>openClient(c.id)}>Obrir</button></div>
</>}</div>})}
</div></Card>}
...
const[certEdits,setCertEdits]=useState(()=>JSON.parse(localStorage.getItem("aco_cert_edits_v43")||"{}"));
...
function qActual(r){let k=`${certNum}_${r.codi}`; if(certEdits[k]!==undefined) return +certEdits[k]||0; return certNum===1?(+r.certAnterior||0):(+r.certActual||0)}
...
function calcCertAmountLive(n){return rows.reduce((s,r)=>{let fb=+n===1?(+r.certAnterior||0):(+r.certActual||0);let k=`${n}_${r.codi}`;let q=certEdits&&certEdits[k]!==undefined?(+certEdits[k]||0):fb;return s+q*(+r.pu||0)},0)}
return <div className="stack">
<Card title="Certificacions guardades"><div className="version-list">{data.certificacions.length===0?<Empty text="Aquesta obra encara no té certificacions guardades."/>:data.certificacions.map(c=><button className={`version-row ${selected===c.id?"active":""}`} key={c.id} onClick={()=>{setSelected(c.id);setView("Quadre detallat")}}><strong>Cert. {c.numero}</strong><span>{c.data}</span><span>{c.estat}</span><b>{money(calcCertAmountLive(+c.numero)||c.import)}</b><em>Obrir quadre</em></button>)}</div></Card>
...
const cfg=getAppConfig();
const initial=()=>JSON.parse(localStorage.getItem("aco_fact_per_proforma_v46")||"{}");
const[per,setPer]=useState(initial);
useEffect(()=>localStorage.setItem("aco_fact_per_proforma_v46",JSON.stringify(per)),[per]);
let certs=(data.certificacions||[]).map(c=>({...c,base:calcEditedCertAmount(data,+c.numero)||(+c.import||0)}));
function paramsFor(id){return per[id]||{iva:cfg.iva||"21",retencio:cfg.retencio||"0",descompte:cfg.descompte||"0"}}
function upd(id,k,v){setPer(p=>({...p,[id]:{...paramsFor(id),[k]:v}}))}
function calc(f){let p=paramsFor(f.id),b=+f.base||0,d=b*(+p.descompte||0)/100,sub=b-d,iva=sub*(+p.iva||0)/100,ret=sub*(+p.retencio||0)/100;return {p,b,d,sub,iva,ret,total:sub+iva-ret}}
function saveOne(id){localStorage.setItem("aco_fact_per_proforma_v46",JSON.stringify(per));alert("Paràmetres de la proforma guardats")}
return <div className="stack"><Card title="Factures proforma de certificacions">{certs.length===0?<Empty text="Encara no hi ha certificacions."/>:<div className="invoice-table-v46">{certs.map(f=>{let x=calc(f),p=x.p;return <div className="proforma-card print-area" key={f.id}><div className="proforma-head"><div><b>Proforma certificació {f.numero}</b><span>{f.data}</span></div><div className="row-actions no-print"><button className="secondary" onClick={()=>saveOne(f.id)}>Guardar paràmetres</button><button className="secondary" onClick={()=>openDoc({type:"proforma",title:`Proforma ${f.numero}`,subtitle:`Certificació ${f.numero} · ${f.data}`,proforma:f,iva:p.iva,ret:p.retencio,ded:p.descompte,total:x.total,linies:[`Base: ${money(x.b)}`,`Deducció (${p.descompte}%): ${money(x.d)}`,`IVA (${p.iva}%): ${money(x.iva)}`,`Retenció (${p.retencio}%): -${money(x.ret)}`,`Total: ${money(x.total)}`]})}>Obrir</button><button className="primary" onClick={()=>window.print()}>Imprimir</button><button className="secondary" onClick={()=>gmailSend(`Proforma certificació ${f.numero}`,`Adjunto/consulta proforma de la certificació ${f.numero}.\n\nBase: ${money(x.b)}\nDeducció (${p.descompte}%): ${money(x.d)}\nIVA (${p.iva}%): ${money(x.iva)}\nRetenció (${p.retencio}%): -${money(x.ret)}\nTotal: ${money(x.total)}\n\nNota: adjuntar manualment el PDF si no hi ha enllaç públic del document.`)}>Enviar Gmail</button></div></div><div className="proforma-params no-print"><label>IVA<select value={p.iva} onChange={e=>upd(f.id,"iva",e.target.value)}><option value="0">0%</option><option value="10">10%</option><option value="21">21%</option></select></label><label>Retenció<select value={p.retencio} onChange={e=>upd(f.id,"retencio",e.target.value)}><option value="0">0%</option><option value="7">7%</option><option value="15">15%</option><option value="19">19%</option></select></label><label>Deducció<select value={p.descompte} onChange={e=>upd(f.id,"descompte",e.target.value)}><option value="0">0%</option><option value="5">5%</option><option value="10">10%</option><option value="15">15%</option></select></label></div><div className="proforma-totals"><span>Base <b>{money(x.b)}</b></span><span>Deducció {p.descompte}% <b>{money(x.d)}</b></span><span>Subtotal <b>{money(x.sub)}</b></span><span>IVA {p.iva}% <b>{money(x.iva)}</b></span><span>Retenció {p.retencio}% <b>-{money(x.ret)}</b></span><strong>Total {money(x.total)}</strong></div></div>})}</div>}</Card></div>}
...
function Agenda({events,openEvent,calM,setCalM,calY,setCalY,selDay,setSelDay}){
const[notes,setNotes]=useState(()=>JSON.parse(localStorage.getItem("aco_calendar_notes_v41")||"[]"));
const[editing,setEditing]=useState(null);
useEffect(()=>localStorage.setItem("aco_calendar_notes_v41",JSON.stringify(notes)),[notes]);
let all=[...(events||[]),...notes],blanks=first(calY,calM),total=days(calY,calM);
let selDate=selDay?new Date(calY,calM,selDay):null;
let sel=selDay?all.filter(e=>e.day===selDay&&e.month===calM&&e.year===calY):[];
function newNote(){setEditing({id:"new",day:selDay||1,month:calM,year:calY,title:"Nova nota",type:"Nota",hora:"09:00",client:"",obra:"",note:"",color:"blue"})}
function save(n){if(n.id==="new")setNotes(p=>[{...n,id:"ag"+Date.now(),month:calM,year:calY},...p]);else setNotes(p=>p.map(x=>x.id===n.id?n:x));setEditing(null)}
function del(id){if(confirm("Segur que vols eliminar aquesta nota?")){setNotes(p=>p.filter(x=>x.id!==id));setEditing(null)}}
return <div className="agenda-grid"><Card title="Agenda / Calendari" action={<button className="primary" onClick={newNote}><Plus/> Nova cita/nota</button>}><div className="calendar-head"><button className="secondary" onClick={()=>calM===0?(setCalM(11),setCalY(calY-1)):setCalM(calM-1)}>‹</button><button className="secondary" onClick={()=>{let d=new Date();setCalM(d.getMonth());setCalY(d.getFullYear());setSelDay(d.getDate())}}>Avui</button><button className="secondary" onClick={()=>calM===11?(setCalM(0),setCalY(calY+1)):setCalM(calM+1)}>›</button><select value={calM} onChange={e=>setCalM(+e.target.value)}>{months.map((m,i)=><option value={i}>{m}</option>)}</select><select value={calY} onChange={e=>setCalY(+e.target.value)}>{years.map(y=><option>{y}</option>)}</select></div><div className="calendar-grid">{["Dl","Dt","Dc","Dj","Dv","Ds","Dg"].map(d=><div className="week" key={d}>{d}</div>)}{Array.from({length:blanks}).map((_,i)=><div className="day blank" key={"b"+i}/>) }{Array.from({length:total}).map((_,i)=>{let day=i+1,ev=all.filter(e=>e.day===day&&e.month===calM&&e.year===calY);return <button className={`day ${selDay===day?"selected":""}`} key={day} onClick={()=>setSelDay(day)}><b>{day}</b>{ev.map(e=><span className={`cal-event ${e.color==="red"?"red":e.color==="orange"?"orange":""}`} onClick={(x)=>{x.stopPropagation();setEditing(e)}}>{e.hora?e.hora+" · ":""}{e.title}</span>)}</button>})}</div></Card><Card title={selDate?dateShort(selDate):"Selecciona un dia"} action={<button className="primary" onClick={newNote}><Plus/> Afegir nota</button>}><div className="day-detail">{selDay&&sel.length===0&&<Empty text="Aquest dia no té cap nota. Pots crear-ne una."/>}{sel.map(e=><button className="event-detail event-click" onClick={()=>setEditing(e)}><strong>{e.title}</strong><span>{e.hora||"—"} · {e.client||"Sense client"} · {e.obra||"Sense obra"}</span><p>{e.note}</p></button>)}</div></Card>{editing&&<div className="note-pop"><div><h3>{editing.id==="new"?"Nova nota":"Editar nota"}</h3><label><span>Data</span><input type="date" value={`${editing.year}-${String(editing.month+1).padStart(2,"0")}-${String(editing.day).padStart(2,"0")}`} onChange={e=>{let d=new Date(e.target.value);setEditing({...editing,day:d.getDate(),month:d.getMonth(),year:d.getFullYear()})}}/></label><label><span>Títol</span><input value={editing.title} onChange={e=>setEditing({...editing,title:e.target.value})}/></label><label><span>Hora</span><input type="time" value={editing.hora||"09:00"} onChange={e=>setEditing({...editing,hora:e.target.value})}/></label><label><span>Client</span><input value={editing.client||""} onChange={e=>setEditing({...editing,client:e.target.value})}/></label><label><span>Obra</span><input value={editing.obra||""} onChange={e=>setEditing({...editing,obra:e.target.value})}/></label><label><span>Observacions</span><textarea value={editing.note||""} onChange={e=>setEditing({...editing,note:e.target.value})}/></label><div className="modal-actions compact">{editing.id!=="new"&&<button className="danger" onClick={()=>del(editing.id)}>Eliminar</button>}<button className="primary" onClick={()=>save(editing)}>Guardar / Tancar</button></div></div></div>}</div>}
...
const key=`aco_honoraris_simple_${obraId||"default"}_v46`;
const[rows,setRows]=useState(()=>JSON.parse(localStorage.getItem(key)||"[]"));
const[editing,setEditing]=useState(null);
const[form,setForm]=useState({data:new Date().toISOString().slice(0,10),tipus:"Pressupost",tasca:"",hores:"1.00",preu:"50.00",km:"0",preuKm:"0.30",altres:"0",concepteAltres:""});
useEffect(()=>localStorage.setItem(key,JSON.stringify(rows)),[rows,key]);
function totalRow(r){return (+r.hores||0)*(+r.preu||0)+(+r.km||0)*(+r.preuKm||0)+(+r.altres||0)}
let totalH=rows.reduce((s,r)=>s+(+r.hores||0),0), totalKm=rows.reduce((s,r)=>s+(+r.km||0),0), total=rows.reduce((s,r)=>s+totalRow(r),0);
function add(){setRows(p=>[{id:"hr"+Date.now(),obraId,...form},...p]);setForm({...form,tasca:"",hores:"1.00",km:"0",altres:"0",concepteAltres:""})}
...
return <div className="stack honoraris-simple"><Card title="Resum temps / cost"><div className="honor-kpis"><Kpi t="HORES" v={`${totalH.toFixed(2)} h`}/><Kpi t="KM" v={`${totalKm.toFixed(2)} km`}/><Kpi t="TOTAL" v={money(total)}/></div></Card><Card title="Nou registre"><div className="honor-form"><label>Data<input type="date" value={form.data} onChange={e=>setForm({...form,data:e.target.value})}/></label><label>Tipologia<select value={form.tipus} onChange={e=>setForm({...form,tipus:e.target.value})}><option>Pressupost</option><option>Certificació d'obra</option><option>Acta d'obra</option><option>Memòria tècnica</option><option>Visita obra</option><option>Gestió administrativa</option><option>Altres</option></select></label><label>Tasca<input value={form.tasca} onChange={e=>setForm({...form,tasca:e.target.value})}/></label><label>Hores<input type="number" step="0.01" value={form.hores} onChange={e=>setForm({...form,hores:e.target.value})}/></label><label>€/h<input type="number" step="0.01" value={form.preu} onChange={e=>setForm({...form,preu:e.target.value})}/></label><label>Km<input type="number" step="0.01" value={form.km} onChange={e=>setForm({...form,km:e.target.value})}/></label><label>€/km<input type="number" step="0.01" value={form.preuKm} onChange={e=>setForm({...form,preuKm:e.target.value})}/></label><label>Altres €<input type="number" step="0.01" value={form.altres} onChange={e=>setForm({...form,altres:e.target.value})}/></label><label>Concepte altres<input value={form.concepteAltres} onChange={e=>setForm({...form,concepteAltres:e.target.value})}/></label></div><div className="card-actions"><button className="primary" onClick={add}>Afegir registre</button></div></Card><Card title="Registres"><div className="time-table-wrap"><table className="time-table"><thead><tr><th>Data</th><th>Tipus</th><th>Tasca</th><th>Hores</th><th>€/h</th><th>Km</th><th>Altres</th><th>Total</th><th>Accions</th></tr></thead><tbody>{rows.length===0&&<tr><td colSpan="9"><Empty text="Encara no hi ha registres."/></td></tr>}{rows.map(r=>{let edit=editing===r.id;return <tr key={r.id}><td>{edit?<input type="date" value={r.data} onChange={e=>upd(r.id,"data",e.target.value)}/>:r.data}</td><td>{edit?<input value={r.tipus} onChange={e=>upd(r.id,"tipus",e.target.value)}/>:r.tipus}</td><td>{edit?<input value={r.tasca} onChange={e=>upd(r.id,"tasca",e.target.value)}/>:r.tasca}</td><td>{edit?<input type="number" step="0.01" value={r.hores} onChange={e=>upd(r.id,"hores",e.target.value)}/>:Number(r.hores||0).toFixed(2)}</td><td>{edit?<input type="number" step="0.01" value={r.preu} onChange={e=>upd(r.id,"preu",e.target.value)}/>:money(r.preu)}</td><td>{edit?<input type="number" step="0.01" value={r.km} onChange={e=>upd(r.id,"km",e.target.value)}/>:Number(r.km||0).toFixed(2)}</td><td>{edit?<input type="number" step="0.01" value={r.altres} onChange={e=>upd(r.id,"altres",e.target.value)}/>:money(r.altres)}</td><td><b>{money(totalRow(r))}</b></td><td><div className="row-actions">{edit?<button className="secondary" onClick={()=>setEditing(null)}>Guardar</button>:<button className="secondary" onClick={()=>setEditing(r.id)}>Editar</button>}<button className="danger" onClick={()=>del(r.id)}>Eliminar</button></div></td></tr>})}</tbody></table></div></Card></div>}
...
function send(){let tos=selected.map(a=>a.email).filter(Boolean).join(",");gmailSend(draft.title,draft.message,tos)}
return <Modal title="Enviar per email" close={close}><div className="form-grid"><label className="span-all"><span>Destinataris</span><div className="check-grid">{agents.length===0?<Empty text="Aquesta obra no té agents amb email assignats."/>:agents.map(a=><label className="check-row" key={a.id}><input type="checkbox" checked={draft.selected.includes(a.id)} onChange={e=>toggle(a.id,e.target.checked)}/><span>{a.nom} · {a.email||"email pendent"}</span></label>)}</div></label><label><span>Assumpte</span><input value={draft.title} onChange={e=>setDraft({...draft,title:e.target.value})}/></label><label className="span-all"><span>Missatge</span><textarea value={draft.message} onChange={e=>setDraft({...draft,message:e.target.value})}/></label><p className="mail-help span-all">S’obrirà Gmail web. Els fitxers no es poden adjuntar automàticament amb aquest sistema; si el document està a Supabase amb URL pública, inclourem/enllaçarem el document. Per adjuntar automàticament i que quedi a “Enviats”, cal Gmail API/OAuth + backend.</p></div><div className="modal-actions"><button className="secondary" onClick={close}>Cancel·lar</button><button className="primary" onClick={send}>Obrir Gmail</button></div></Modal>}
