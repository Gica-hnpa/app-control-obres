...
function gmailSendV50(subject,body,to=""){
  const url="https://mail.google.com/mail/?view=cm&fs=1"
    +"&to="+encodeURIComponent(to||"")
    +"&su="+encodeURIComponent(subject||"")
    +"&body="+encodeURIComponent(body||"");
...
const[per,setPer]=useState(()=>{try{return JSON.parse(localStorage.getItem("aco_proformes_v50")||"{}")}catch(e){return {}}});
const[printId,setPrintId]=useState(null);
useEffect(()=>localStorage.setItem("aco_proformes_v50",JSON.stringify(per)),[per]);
let certs=(data.certificacions||[]).map(c=>({...c,base:+c.import||0}));
...
const key=`aco_temps_invertit_${obraId||"default"}_v50`;
...
return <div className="stack temps-v50">
<Card title="Temps invertit / honoraris i despeses">
...
<Card title="Registres de temps invertit">
<div className="time-table-wrap"><table className="time-table">
<thead><tr><th>Data</th><th>Tipus</th><th>Tasca</th><th>Hores</th><th>€/h</th><th>Km</th><th>Altres</th><th>Total</th><th>Accions</th></tr></thead>
<tbody>{rows.length===0&&<tr><td colSpan="9"><Empty text="Encara no hi ha registres."/></td></tr>}
