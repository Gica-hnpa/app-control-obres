...
function certStoreKey(obraId){return `aco_cert_store_${obraId||"default"}_v48`}
function loadCertStore(obraId){
  try{return JSON.parse(localStorage.getItem(certStoreKey(obraId))||"{}")}catch(e){return {}}
}
function saveCertStore(obraId,store){
  localStorage.setItem(certStoreKey(obraId),JSON.stringify(store||{}));
}
function calcCertImportFromRows(rows,certNum,store){
  return (rows||[]).reduce((s,r)=>{
    const fallback=+certNum===1?(+r.certAnterior||0):(+r.certActual||0);
    const q=store?.[certNum]?.[r.codi]!==undefined ? (+store[certNum][r.codi]||0) : fallback;
    return s + q*(+r.pu||0);
  },0);
}
function getCertQtyFromStore(row,certNum,store){
  const fallback=+certNum===1?(+row.certAnterior||0):(+row.certActual||0);
  return store?.[certNum]?.[row.codi]!==undefined ? (+store[certNum][row.codi]||0) : fallback;
}
...
function saveCert(numOverride){
  ...
  const store=loadCertStore(obraId);
  const certData=(data.certificacions||[]).find(c=>String(c.numero)===n);
  const total=calcCertImportFromRows(data.partides,+n,store);
...
class SafePanel extends React.Component{
  ...
  render(){
    if(this.state.err){
      return <div className="safe-error"><h3>Apartat bloquejat</h3><p>{String(this.state.err?.message||this.state.err)}</p><button onClick={()=>this.setState({err:null})}>Reintentar</button></div>
    }
...
const[certStore,setCertStore]=useState(()=>loadCertStore(obraId));
...
function qActual(r){return getCertQtyFromStore(r,certNum,certStore)}
...
return <div className="stack">
<Card title="Certificacions guardades"><div className="version-list">{data.certificacions.length===0?<Empty text="Aquesta obra encara no té certificacions guardades."/>:data.certificacions.map(c=><button className={`version-row ${selected===c.id?"active":""}`} key={c.id} onClick={()=>{setSelected(c.id);setView("Quadre detallat")}}><strong>Cert. {c.numero}</strong><span>{c.data}</span><span>{c.estat}</span><b>{money(calcCertImportFromRows(rows,+c.numero,certStore)||c.import)}</b><em>Obrir quadre</em></button>)}</div></Card>
...
const cfg=getAppConfig();
const store=loadCertStore(data.id||data.obraId||"default");
const[per,setPer]=useState(()=>{try{return JSON.parse(localStorage.getItem("aco_fact_per_proforma_v48")||"{}")}catch(e){return {}}});
const[printId,setPrintId]=useState(null);
useEffect(()=>localStorage.setItem("aco_fact_per_proforma_v48",JSON.stringify(per)),[per]);
let certs=(data.certificacions||[]).map(c=>({...c,base:calcCertImportFromRows(data.partides,+c.numero,store)||(+c.import||0)}));
...
function saveOne(id){localStorage.setItem("aco_fact_per_proforma_v48",JSON.stringify(per));alert("Paràmetres de la proforma guardats")}
function printOne(id){setPrintId(id);setTimeout(()=>{window.print();setTimeout(()=>setPrintId(null),500)},80)}
return <div className="stack fact-v48"><Card title="Factures proforma de certificacions">
{certs.length===0?<Empty text="Encara no hi ha certificacions."/>:<div className="proforma-list-v48">{certs.map(f=>{let x=calc(f),p=x.p;return <div className={`proforma-card-v48 ${printId===f.id?"print-target":""}`} key={f.id}>
<div className="proforma-head-v48"><div><b>Proforma certificació {f.numero}</b><span>{f.data}</span></div><div className="row-actions no-print"><button className="secondary" onClick={()=>saveOne(f.id)}>Guardar paràmetres</button><button className="primary" onClick={()=>printOne(f.id)}>Imprimir / PDF</button><button className="secondary" onClick={()=>gmailSend(`Proforma certificació ${f.numero}`,`Adjunto/consulta proforma de la certificació ${f.numero}.\n\nBase: ${money(x.b)}\nDeducció (${p.descompte}%): ${money(x.d)}\nSubtotal: ${money(x.sub)}\nIVA (${p.iva}%): ${money(x.iva)}\nRetenció (${p.retencio}%): -${money(x.ret)}\nTotal: ${money(x.total)}\n\nNota: cal adjuntar manualment el PDF si no hi ha enllaç públic del document.`)}>Enviar Gmail</button></div></div>
<div className="proforma-params-v48 no-print"><label>IVA<select value={p.iva} onChange={e=>upd(f.id,"iva",e.target.value)}><option value="0">0%</option><option value="10">10%</option><option value="21">21%</option></select></label><label>Retenció<select value={p.retencio} onChange={e=>upd(f.id,"retencio",e.target.value)}><option value="0">0%</option><option value="7">7%</option><option value="15">15%</option><option value="19">19%</option></select></label><label>Deducció<select value={p.descompte} onChange={e=>upd(f.id,"descompte",e.target.value)}><option value="0">0%</option><option value="5">5%</option><option value="10">10%</option><option value="15">15%</option></select></label></div>
<table className="proforma-table-v48"><tbody><tr><th>Concepte</th><th>Import</th></tr><tr><td>Base certificada</td><td>{money(x.b)}</td></tr><tr><td>Deducció ({p.descompte}%)</td><td>- {money(x.d)}</td></tr><tr><td>Subtotal</td><td>{money(x.sub)}</td></tr><tr><td>IVA ({p.iva}%)</td><td>{money(x.iva)}</td></tr><tr><td>Retenció ({p.retencio}%)</td><td>- {money(x.ret)}</td></tr><tr className="total"><td>Total proforma</td><td>{money(x.total)}</td></tr></tbody></table>
</div>})}</div>}</Card></div>}
...
const key=`aco_honoraris_v48_${obraId||"default"}`;
...
return <div className="stack honoraris-v48">
<Card title="Temps invertit / cost treball"><div className="honor-kpis"><Kpi t="HORES" v={`${totalH.toFixed(2)} h`}/><Kpi t="KM" v={`${totalKm.toFixed(2)} km`}/><Kpi t="TOTAL" v={money(total)}/></div></Card>
<Card title="Nou registre"><div className="honor-form-v48">
...
</div><div className="card-actions"><button className="primary" onClick={add}>Afegir registre</button></div></Card>
<Card title="Registres"><div className="time-table-wrap"><table className="time-table"><thead><tr><th>Data</th><th>Tipus</th><th>Tasca</th><th>Hores</th><th>€/h</th><th>Km</th><th>Altres</th><th>Total</th><th>Accions</th></tr></thead><tbody>{rows.length===0&&<tr><td colSpan="9"><Empty text="Encara no hi ha registres."/></td></tr>}{rows.map(r=>{let editing=edit===r.id;return <tr key={r.id}>
