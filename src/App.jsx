...
function mailtoSend(subject,body,to=""){
  const cfg=getAppConfig();
  const from=cfg.email||"";
  const txt=(body||"") + (from?`%0A%0AEmissor configurat: ${encodeURIComponent(from)}`:"");
  window.location.href=`mailto:${encodeURIComponent(to)}?subject=${encodeURIComponent(subject||"")}&body=${txt}`;
}
...
return <div className="stack">
<Card title="Certificacions guardades"><div className="version-list">{data.certificacions.length===0?<Empty text="Aquesta obra encara no té certificacions guardades."/>:data.certificacions.map(c=><button className={`version-row ${selected===c.id?"active":""}`} key={c.id} onClick={()=>{setSelected(c.id);setView("Quadre detallat")}}><strong>Cert. {c.numero}</strong><span>{c.data}</span><span>{c.estat}</span><b>{money(calcEditedCertAmount(data,+c.numero)||c.import)}</b><em>Obrir quadre</em></button>)}</div></Card>
...
const cfg=getAppConfig();
const[params,setParams]=useState(()=>JSON.parse(localStorage.getItem("aco_fact_params_v44")||localStorage.getItem("aco_fact_params_v43")||localStorage.getItem("aco_fact_params_v41")||"null")||{iva:cfg.iva||"21",retencio:cfg.retencio||"0",descompte:cfg.descompte||"0"});
useEffect(()=>localStorage.setItem("aco_fact_params_v44",JSON.stringify(params)),[params]);
let certs=(data.certificacions||[]).map(c=>({...c,import:calcEditedCertAmount(data,+c.numero)||(+c.import||0)}));
function upd(k,v){setParams(p=>({...p,[k]:v}))}
function calc(c){let b=+c.import||0,d=b*(+params.descompte||0)/100,sub=b-d,iva=sub*(+params.iva||0)/100,ret=sub*(+params.retencio||0)/100;return {b,d,sub,iva,ret,total:sub+iva-ret}}
function save(){localStorage.setItem("aco_fact_params_v44",JSON.stringify(params));alert("Paràmetres de facturació guardats")}
return <div className="stack">
<Card title="Paràmetres factura / proforma" action={<button className="primary" onClick={save}><Save/> Guardar paràmetres</button>}>
<div className="fact-param-grid"><label><span>IVA %</span><input type="number" step="0.01" value={params.iva} onChange={e=>upd("iva",e.target.value)}/></label><label><span>Retenció %</span><input type="number" step="0.01" value={params.retencio} onChange={e=>upd("retencio",e.target.value)}/></label><label><span>Descompte / deducció %</span><input type="number" step="0.01" value={params.descompte} onChange={e=>upd("descompte",e.target.value)}/></label></div>
<p className="mail-help">Email emissor configurat: <b>{cfg.email||"pendent de configurar"}</b>. Ara l’enviament és via mailto. Per Gmail real i veure’l a “Enviats” cal integrar Gmail API/OAuth amb backend.</p>
</Card>
<Card title="Llistat proformes">{certs.length===0?<Empty text="Encara no hi ha certificacions per facturar."/>:<div className="fact-table">{certs.map(c=>{let x=calc(c);return <div className="fact-row print-area" key={c.id}><strong>Proforma certificació {c.numero}</strong><span>{c.data}</span><span>Base: {money(x.b)}</span><span>Descompte ({params.descompte}%): {money(x.d)}</span><span>Subtotal: {money(x.sub)}</span><span>IVA ({params.iva}%): {money(x.iva)}</span><span>Retenció ({params.retencio}%): -{money(x.ret)}</span><b>Total: {money(x.total)}</b><button className="secondary no-print" onClick={()=>mailtoSend(`Proforma certificació ${c.numero}`,`Adjunto/consulta proforma de la certificació ${c.numero}.%0ABase: ${money(x.b)}%0ADescompte: ${money(x.d)}%0AIVA: ${money(x.iva)}%0ARetenció: -${money(x.ret)}%0ATotal: ${money(x.total)}`)}>Enviar</button><button className="primary no-print" onClick={()=>window.print()}>Imprimir</button></div>})}</div>}</Card></div>}
